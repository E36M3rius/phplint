...
class LintCommand extends Command
{
    ...
    protected function getScreenColumns()
    {
        ...
        if (function_exists('shell_exec') && preg_match('#\d+ (\d+)#', shell_exec('unbuffer stty size'), $match) === 1) {
            if ((int) $match[1] > 0) {
                return (int) $match[1];
            }
        }

        if (function_exists('shell_exec') && preg_match('#columns = (\d+);#', shell_exec('unbuffer stty'), $match) === 1) {
            if ((int) $match[1] > 0) {
                return (int) $match[1];
            }
        }
